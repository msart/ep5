/****************************************************/
/* Exercicio-Programa 5 - MAC0122 		    */
/* Nome: Gustavo Rodrigues Cayres Silva Nº: 8584323 */
/* Nome: Pedro Marcondes Nº: 8941168                */
/* Nome: Ricardo Stefan Devai Melo Nº: 89441297	    */
/****************************************************/
...
int inimigoAdjacente(Position pos, Grid *grid, Robot *r)
{
	...
	for (i = 0; i < 6; i++){
		if (adjacentes[i] == ROBOT){
			...
			if ((rob->dir + 3)%6 == i || (rob->dir + 2)%6 == i || (rob->dir + 4)%6 == i){
				return 1;
			}
		}
	}
...
/* checa se a posicao p contem um block */
int isBlock (Grid *g, Position p) {
...
int searchNearestProjectile (Grid *g, Position p, Robot *r) {
	...
	for (i = 0; i < 6; i++) {
		...
		cont = 1 + quickTurn (r->dir, i);
		s = getNeighbor (p, i);
		proj = &g->map[s.x][s.y].object.projectile;
		while (limiteDoMapa (s, g->m, g->n)) {
			if (isBlock(g, s))
				break;
			else if (isProjectile(g, s) && (proj->dir + 3)%6 == i) {
				if(cont < min) {
					min = cont;
					best_dir = i;
					break;
...
Action processTurn(Grid *g, Position p, int turnsLeft) {
	...
	if (estadoVida == SAUDAVEL)
		return acaoSaudavel(g, p);
	else if (estadoVida == RISCO)
